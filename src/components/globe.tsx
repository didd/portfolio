...
export function Globe() {
  ...
  const globeReadyBeforeMountRef = useRef(false);
  ...
  useEffect(() => {
    ...
    if (globeReadyBeforeMountRef.current) {
      setIsGlobeReady(true);
      globeReadyBeforeMountRef.current = false;
    }
    ...
    const loadCountryPolygons = async (): Promise<void> => {
      try {
        ...
        if (!isCancelled && isMountedRef.current) {
          setCountryFeatures(safeCountryFeatures);
        }
      } catch (error) {
  ...
  const handleGlobeReady = useCallback((): void => {
    if (isMountedRef.current) {
      ...
      globeReadyBeforeMountRef.current = true;
    }
    const controls = globeRef.current?.controls();
    if (controls) {
